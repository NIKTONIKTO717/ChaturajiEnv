...
BUDGET = [400, 800, 1600, 3200] # Number of MCTS simulations per move (800 in AlphaZero)
...
def main():
    ...
    with open('vs_vanilla_mcts_rewards_gens_array.pkl', 'wb') as f:
        pickle.dump(rewards_gens_array, f)
    with open('vs_vanilla_mcts_moves_sum_array.pkl', 'wb') as f:
        pickle.dump(moves_sum_array, f)
    with open('vs_vanilla_mcts_score_sum_array.pkl', 'wb') as f:
        pickle.dump(score_sum_array, f)

    rewards_by_agent = list(zip(*rewards_gens_array))
    plt.plot(rewards_by_agent[0], label='no_eval')
    plt.plot(rewards_by_agent[1], label='MCTS, budget=12800')
    plt.plot(rewards_by_agent[2], label='MCTS, budget=25600')
    plt.plot(rewards_by_agent[3], label='MCTS, budget=51200')
